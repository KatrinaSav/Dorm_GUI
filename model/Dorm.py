...
class Dorm:
    ...
    def kick_student(self, student: Student, roomy: Room):
        ...
        return "Студент " + student.get_name()+" был выселен из общежития\n"

    def check_in(self, roomy: Room):
        student = Student()
        if roomy.get_number_of_students() >= 2:
            return 'Максимальное количество студентов в комнате 2'
        else:
            roomy.add_student(student)
            self.__student_list.append(student)
            return "__Заселённый студент__\n" + student.info() + "\n______________________"
    ...
    def roach(self):
        ...
        for each_room in rooms_with_roaches:
            ...
            return 'В комнате №'+ str(each_room.get_number_of_room()) + 'завелись тараканы'
            ...
            for each_student in each_room.get_students():
                if self.__month == 1:
                    if not each_student.exams():
                        message = message + "Студент " + each_student.get_name() + " завалил сессию и был отчислен\n"
    ...
    def info(self):
        info = 'Информация о состоянии общежития\n' + 'Дежурная комната: ' + str(self.__index_of_duty) + \
               '\nТекущий месяц: ' + str(self.__month) + '\nУровень счастья: ' + \
               "{:4.2f}".format(self.calculate_level_of_happiness()) + ' из 300\n' + 'Количество студентов: ' + \
               str(len(self.__student_list))
        if self.__is_kitchen_okey:
            info = info + '\nКухня в прекрасном состоянии'
        else:
            info = info + '\nКухня в печальном состоянии'
        return info
